...
//            vcDelegate?.movedPiece(color: game.piece(at: move.toPos)?.pieceColor ?? .white, move: move)
    ...
    private func addRedCell(at pos: Pos, check: Bool) {
        ...
        redCell.fillColor = .init(rgb: 0xFF7157)
    ...
    func displayMoveArrows(moves: [NormalMove]) {
        ...
        for (i, move) in moves.enumerated() {
            let (x1,y1,x2,y2) = (move.fromPos.x, move.fromPos.y, move.toPos.x, move.toPos.y)
            
            let arrow = SKShapeNode(arrowWithFillColor: .init(red: 0.41015625, green: 0.26171875, blue: 0.15234375, alpha: highlightFirstArrow && i == 0 ? 1 : 0.5),
                                    strokeColor: .clear,
                                    lineWidth: 0,
                                    length: sqrt(pow(Double(x1 - x2), 2) + pow(Double(y1 - y2), 2)) * cellSize,
                                    tailWidth: cellSize * 0.12,
                                    headWidth: cellSize * 0.45,
                                    headLength: cellSize * 0.37)
            arrow.zRotation = atan2(Double(y1 - y2), Double(x2 - x1)) - .pi / 2
            arrow.position = positionInBoard(at: move.fromPos)
            arrow.zPosition = 0.95
            moveArrows?.append(arrow)
            addChild(arrow)
        }
    }
    func setHighlightFirstArrow(_ on: Bool) {
        guard !(moveArrows?.isEmpty ?? true) else { return }
        highlightFirstArrow = on
        moveArrows![0].fillColor = .init(red: 0.41015625, green: 0.26171875, blue: 0.15234375, alpha: on ? 1 : 0.5)
    }
    ...
    func createBoardImage() {
        ...
        if alignToTop {
            boardImage?.position = .init(x: 0, y: self.frame.height / 2 - width / 2)
        }
        addChild(boardImage!)
    }
...
//        vcDelegate?.checkmate(wins: wins)
    ...
    func pawnReachedEnd(color: ChessPieceColor, completion: @escaping (ChessPieceType) -> ()) {
        ...
        alert.addAction(UIAlertAction(title: "Queen".localized, style: .default, handler: { _ in
            callback(.queen)
        }))
        alert.addAction(UIAlertAction(title: "Knight".localized, style: .default, handler: { _ in
            callback(.knight)
        }))
        alert.addAction(UIAlertAction(title: "Bishop".localized, style: .default, handler: { _ in
            callback(.bishop)
        }))
        alert.addAction(UIAlertAction(title: "Rook".localized, style: .default, handler: { _ in
            callback(.rook)
        }))
